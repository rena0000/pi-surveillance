...
def detect_motion():
    ...
    while True:
        ...
        frame_resized = cv.resize(frame_colour, (640, 480))
        dilated_resized = cv.resize(dilated_frame, (640, 480))
        # Set text and timestamp
        cv.putText(frame_resized, "Status: {}".format(motion_status_text), (10,20), cv.FONT_HERSHEY_DUPLEX, 0.5, (0, 0, 255), 2)
        cv.putText(frame_resized, get_date_time(), (10, frame_colour.shape[0] - 10), cv.FONT_HERSHEY_DUPLEX, 0.35, (0, 0, 255), 1)
        # Show windows
        cv.imshow("Feed", frame_resized)
        cv.imshow("Delta", dilated_resized)
